...
begin
    ...
    if x1 then work=work-work div x;
end;
procedure dfs(t,sumlongint; x,yint64);
 var ilongint;
begin
    if ts then
    begin
        if sum=2 then inc(ans) else inc(ans,sumx div ysum div 2);
        exit;
    end;
    dfs(t+1,sum,x,y);
    for i=1 to a[t,2] do
