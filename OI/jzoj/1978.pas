...
var	n,m,i,j,x,y,pi,ans:longint;
...
begin
	...
	l:=p[k].l;r:=p[k].r;
	x:=p[k].x;
	if (i=y) and (j=z) then ans:=ans+x else begin
		if (y>=p[l].i) and (z<=p[l].j) then search(l,y,z);
		if (y>=p[r].i) and (z<=p[r].j) then search(r,y,z);
		if (y<=p[l].j) and (z>=p[r].i) then begin
                        search(l,y,p[l].j);
                        search(r,p[r].i,z);
                end;
	end;
