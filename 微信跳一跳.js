...
//按压位置为再来一局的位置
var press_x = device.width / 2;
var press_y = 1584 * (device.height / 1920.0);
const piece_body_width = 55       // 棋子的宽度，比截图中量到的稍微大一点比较安全，可能要调节
const piece_dist_from_top_to_base = 188;  //188棋子最顶部到棋子底部中点的距离
...
function jump(distance){
    var tmp = 0;
    if (distance<300) tmp = 0.06;
    if (distance<200) tmp = 0.20;
    if (distance<175) tmp = 0.25;
    if (distance>350) tmp = 0.03;
    if (distance>400) tmp = -0.06;
    // 跳跃参数调整
    
    var press_time = distance * (press_coefficient+tmp);
    press_time = max(200, press_time);
    press_compat(press_x, press_y, parseInt(press_time));
    log("distance:",distance,",press time:",press_time);
    toast("距离:"+parseInt(distance)+",按压时间:"+parseInt(press_time));
    //sleep(press_time)+300;
}
...
function find_board(im, piece){
    var board_x = 0;
    var board_y = 0;
    for(var i = parseInt(h / 3); i < parseInt(h * 2 / 3); i += 5){
...
function find_piece(im){
    ...
    var piece_top_center_x = piece_start_x + 25;
